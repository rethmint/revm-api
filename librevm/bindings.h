...
enum GoError {
  ...
  /**
   * An error happened during normal operation of a Go callback, which should be
   * fed back to the contract
   */
  GoError_User = 4,
  /**
   * Unimplemented
  ...
  /**
   * An error type that should never be created by us. It only serves as a
   * fallback for the i32 to GoError conversion.
   */
  GoError_Other = -1,
};
 ...
 * Use this for the current call only. A view cannot be copied for safety
 * reasons. If you need a copy, use [`ByteSliceView::to_owned`].
 *
 * Go's nil value is fully supported, such that we can differentiate between nil
 * and an empty slice.
 */
typedef struct {
  /**
   * True if and only if the byte slice is nil in Go. If this is true, the other
 ...
 * It can be created from `Option<Vec<u8>>` and be converted into
 * `Option<Vec<u8>>`.
 *
 ...
 * [`UnmanagedVector::default`] or [`new_unmanaged_vector`], it can be passted
 * to Go as a return value. Rust then has no chance to destroy the vector
 * anymore, so ownership is transferred to Go. In Go, the data has to be copied
 * to a garbage collected `[]byte`. Then the vector must be destroyed using
 * [`destroy_unmanaged_vector`].
 *
 * ### Transfer ownership from Go to Rust
 *
 * When Rust code calls into Go (using the vtable methods), return data or error
 * messages must be created in Go. This is done by calling
 * [`new_unmanaged_vector`] from Go, which copies data into a newly created
 * `UnmanagedVector`. Since Go created it, it owns it. The ownership is then
 * passed to Rust via the mutable return value pointers. On the Rust side, the
 * vector is destroyed using [`UnmanagedVector::consume`].
 *
 */
typedef struct {
  /**
   * True if and only if this is None. If this is true, the other fields must be
   * ignored.
   */
  bool is_none;
  uint8_t *ptr;
...
UnmanagedVector execute_tx(evm_t *vm_ptr, Db db, ByteSliceView block,
                           ByteSliceView tx);

evm_t *init_vm(void);

UnmanagedVector new_unmanaged_vector(bool nil, const uint8_t *ptr,
                                     size_t length);

UnmanagedVector query(evm_t *vm_ptr, Db db, ByteSliceView block,
                      ByteSliceView tx);
